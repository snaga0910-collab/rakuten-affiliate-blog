...
const TAGS = {
  ...
  "air-fryer": ["エアフライヤー", "ノンフライヤー", "キッチン家電", "一人暮らし", "買ってよかったもの"],
  "sous-vide": ["低温調理器", "鶏むね", "サラダチキン", "自炊", "一人暮らし"],
  "yoshikei": ["ヨシケイ", "食材宅配", "ミールキット", "一人暮らし", "時短"],
  "tsukurioki": ["シェフの無添つくりおき", "食材宅配", "作り置き", "一人暮らし", "時短"],
  ...
  "interdental-brush": ["歯間ブラシ", "オーラルケア", "デンタルケア", "一人暮らし", "買ってよかったもの"],
  "water-cost": ["浄水器", "ウォーターサーバー", "一人暮らし", "節約", "暮らしの工夫"],
};
...
function tableToList(lines) {
  ...
  for (const row of body) {
    ...
    // 商品ごとのURLを、その商品のすぐ下に置く。
    //
    // 2026-09-12: もともと「生URLが6本並ぶと読めない」という理由で本文からは
    // 落とし、記事末の一覧に集約していた。しかし運営者から
    // 「ここにリンクが無いのは気持ちが悪い。活字だけで買いたいとは思えない」
    // と指摘があり、商品の真横に置く形へ変更した。読みやすさより、
    // 読んだその場で買えることを優先する。
    //
    // note は「前後が空行の単独URL」だけをリンクにするので、空行で挟む。
    if (href && !isAdUrl(href[1])) {
      out.push("", href[1]);
    }
    out.push("");
  }
...
const NOTE_NG = [
  ...
  // 2026-09-03: 表のセルの改行に <br> を使ったら、note にそのまま出た。
  // 表は「◾️ 商品名／項目 値」に展開されるので、セル内改行はもともと要らない。
  [/<br\s*\/?>/, "改行タグ（表のセルで <br> を使わない）"],
];
let ngCount = 0;
